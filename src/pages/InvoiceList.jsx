...
import { addBulkInvoices } from "../redux/bulkInvoicesSlice";
...
const InvoiceList = () => {
  ...
  const handleBulkUpdate = () => {
    if(!multiEdit.length) {
      alert('Select atleast one to bulk update')
    } else {
      dispatch(addBulkInvoices(multiEdit))
        ...
        <Card className="d-flex p-3 p-md-4 my-3 my-md-4 ">
          {isListEmpty ? (
            ...
            <div className="d-flex flex-column">
              ...
              {
                <Button variant="success" onClick={handleBulkUpdate}>Bulk Update</Button>
              }
            </div>
          )}
        </Card>
...
const InvoiceRow = ({ invoice, navigate, multiEdit, setMultiEdit }) => {
  ...
  const handleCheck = (id, state, multiEdit, setMultiEdit) => {
    if(state) {
      multiEdit.push(id)
    } else {
      const index = multiEdit.indexOf(id)
      if(index !== -1)
        multiEdit.splice(index, 1)
    }
  ...
  return (
    <tr>
      <td><input type="checkbox" defaultValue={false} onChange={(e) => handleCheck(invoice.id, e.target.checked, multiEdit, setMultiEdit)}/></td>
